...
public:
	...
	//CONV1 memory address
	addr_t conv1_core0_start_addr = 0x73000000;
	addr_t conv1_core0_size = 0x00100000;
	addr_t conv1_core0_end_addr = conv1_core0_start_addr + conv1_core0_size - 1;
	//CONV2 memory address
	addr_t conv2_core0_start_addr = 0x73200000;
	addr_t conv2_core0_size = 0x00100000;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > fc3_core0_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	Convolution_1 conv1_core0("conv1_core0");
	...
	bus.ports[12] = new PortMapping(opt.conv1_core0_start_addr, opt.conv1_core0_end_addr);
	bus.ports[13] = new PortMapping(opt.conv2_core0_start_addr, opt.conv2_core0_end_addr);
	bus.ports[14] = new PortMapping(opt.fc1_core0_start_addr, opt.fc1_core0_end_addr);
	bus.ports[15] = new PortMapping(opt.fc2_core0_start_addr, opt.fc2_core0_end_addr);
	bus.ports[16] = new PortMapping(opt.fc3_core0_start_addr, opt.fc3_core0_end_addr);
	
	// connect TLM sockets
	core0_mem_if.isock.bind(bus.tsocks[0]);
	core0_dbg_if.isock.bind(bus.tsocks[1]);
	...
	bus.isocks[13].bind(conv2_core0.tsock);
	bus.isocks[14].bind(fc1_core0.tsock);
	bus.isocks[15].bind(fc2_core0.tsock);
	bus.isocks[16].bind(fc3_core0.tsock);
