...
public:
	...
	addr_t filter_start_addr = 0x73000000;
	addr_t filter_size = 0x01000000;
	addr_t filter_end_addr = filter_start_addr + filter_size - 1;
	addr_t filter_RS_start_addr = 0x74000000;
	addr_t filter_RS_size = 0x01000000;
	addr_t filter_RS_end_addr = filter_RS_start_addr + filter_RS_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > filter_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[12] = new PortMapping(opt.filter_start_addr, opt.filter_end_addr);
	//bus.ports[13] = new PortMapping(opt.filter_RS_start_addr, opt.filter_RS_end_addr);
	...
	bus.isocks[12].bind(filter.tsock);
	//bus.isocks[13].bind(filter_rs.tsock);
